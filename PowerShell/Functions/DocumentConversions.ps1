...
Function Convert-HTMLToPDF()
{
    ...
    If ($Path -eq $null -or [String]::IsNullOrWhiteSpace($Path))
    {
        ...
        [void] [IO.File]::Move($Path, [IO.Path]::ChangeExtension($Path, "html"));
        $Path = [IO.Path]::ChangeExtension($Path, "html");
        [void] [IO.File]::WriteAllText($Path, $HTML);
	}
    ...
    If ($IsTemFile -and [IO.File]::Exists($Path))
    {
        [IO.File]::Delete($Path);
    }
}
